...
namespace Exwhyzee.AhiomaDashboard.MainWebsite.Areas.Dashboard.Pages.Analysis
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task OnGetAsync(DateTime? DateOne, DateTime? DateTwo, string Duration = "All")
        {
            ...
            try
            {
                IQueryable<OrderItem> orderitems = from s in _context.OrderItems
                                                   select s;
                if (Duration != "All")
                {
                ...
                PendingOrder = orderitems.Where(x => x.Status == Enums.OrderStatus.Processing).Count();
                AwaitingConfirmation = orderitems.Where(x => x.Status == Enums.OrderStatus.Reversed).Count();
