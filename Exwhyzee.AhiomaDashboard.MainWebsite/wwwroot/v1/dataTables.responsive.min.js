...
//        a) return b[d]
//    }, _redrawChildren: function () { var a = this, b = this.s.dt; b.rows({ page: "current" }).iterator("row", function (d, c) { b.row(c); a._detailsDisplay(b.row(c), !0) }) }, _resize: function (a) {
//        var b = this, d = this.s.dt, e = c(m).innerWidth(), f = this.c.breakpoints, g = f[0].name, k = this.s.columns, h, i = this.s.current.slice(); for (h = f.length - 1; 0 <= h; h--)if (e <= f[h].width) { g = f[h].name; break } var j = this._columnsVisiblity(g); this.s.current = j; f = !1; h = 0;
//        for (e = k.length; h < e; h++)if (!1 === j[h] && !k[h].never && !k[h].control && !1 === !d.column(h).visible()) {
//            f =
//        !0; break
//    } c(d.table().node())
//            .toggleClass("collapsed", f); var l = !1, n = 0; d.columns().eq(0).each(function (d, c) { !0 === j[c] && n++; if (a || j[c] !== i[c]) l = !0, b._setColumnVis(d, j[c]) }); l && (this._redrawChildren(), c(d.table().node()).trigger("responsive-resize.dt", [d, this.s.current]), 0 === d.page.info().recordsDisplay && c("td", d.table().body()).eq(0).attr("colspan", n));
//        if ("inline" === this.c.details.type) {
//            var o = c.inArray(!0, j); d.cells(null, function (a) { return a !== o }, { page: "current" }).nodes().to$().filter(".dtr-control").removeClass("dtr-control");
