...
{
    public class CustomerPrintViewComponent : ViewComponent
    {
        private readonly AhiomaDbContext _context;
        private readonly IHostingEnvironment _hostingEnv;
        public CustomerPrintViewComponent(AhiomaDbContext context, IHostingEnvironment hostingEnv)
        {
            _context = context;
        ...
        public async Task<IViewComponentResult> InvokeAsync(long id)
        {
           ...
           var AmountPaidForOrders = await OrderItem.Where(x => x.Status == Enums.OrderStatus.Reversed || x.Status == Enums.OrderStatus.Completed || x.Status == Enums.OrderStatus.Processing).SumAsync(x=>x.Product.Price);
            ...
            ViewBag.total = OrderItems.Sum(x => x.Product.Price) + Order.LogisticAmount;
